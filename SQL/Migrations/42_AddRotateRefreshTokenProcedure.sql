...
BEGIN
    ...
    IF @OldTokenHash IS NULL OR LTRIM(RTRIM(@OldTokenHash)) = ''
    BEGIN
        SELECT
    ...
    IF @NewTokenHash IS NULL OR LTRIM(RTRIM(@NewTokenHash)) = ''
    BEGIN
        SELECT
            CAST(0 AS BIT) AS IsSuccess,
    ...
    BEGIN TRY
        ...
        SET @NewRefreshTokenId = CAST(SCOPE_IDENTITY() AS INT);
